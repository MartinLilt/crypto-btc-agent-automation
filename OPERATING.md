...
| **tracker** cron | Railway eu-west, `python -m research.forward step` daily 00:05 UTC | Live P&L of hardened 50/50 book (rotation+carry) on `/app/data/forward_state.json` |
...
| Tracker showing `no new bar since...` for >2 days | Binance kline endpoint down (rare) or cron misfired — check `railway status` for cron schedule |
