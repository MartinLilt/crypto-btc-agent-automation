...
### Что работает: свой egress

25.08 прилетел **третий** бан подряд (53 мин), и стало окончательно ясно, что
внутри Railway это не чинится — выделенного IP они не дают в принципе. Решение:
весь трафик к Binance уходит через коробку с собственным IPv4
(`BINANCE_PROXY_URL`, точка входа — `src/net.py`). На адресе, где кроме нас
никого, наши ~38 единиц веса против 6000 в минуту забанить нечем.

Побочно: в белый список ключа идёт **один** свой адрес вместо трёх общих, и
Pro-тариф Railway ($20, нужен был только ради Static Outbound IPs) меняется на
Hobby ($5).

Пошаговая установка и порядок переключения: [[Dedicated egress IP]].
...
- [x] Свой egress IP вместо общего — [[Dedicated egress IP]] *(код готов 25.08,
      коробку ещё надо купить)*
- [x] Потолок ожидания бана — бюджет на весь запуск, а не на каждый вызов
      (иначе поднятый `BAN_MAX_WAIT_MIN` наезжает на следующий слот cron)
- [ ] Выключить право на вывод у боевого ключа
- [ ] Внести `<vps-ip>` в белый список ключа (и убрать адреса Railway после
      2–3 чистых суток)
